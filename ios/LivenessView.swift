...
class LivenessView: UIView, LivenessUtilityDetectorDelegate {
    ...
    Task {
      do {
        if isThreeDimension == true {
          ...
          self.livenessDetector = LivenessUtil.createLivenessDetector(previewView: self, threshold: .low,delay: 0, smallFaceThreshold: minFaceSize, debugging: self.debugging, delegate: self, livenessMode: stateFaceId ? .threeDimension : .twoDimension)
        } else {
  ...
  func liveness(liveness: LivenessUtility.LivenessUtilityDetector, didFinish verificationImage: UIImage, thermalImage: UIImage?, color: String?, videoURL: URL?) {
    Task {
      ...
      let image1 = compressedImage?.jpegData(compressionQuality: 1)!
      ...
      if stateFaceId == true {
        if thermalImage != nil {
          ...
          let image2 = compressedImageThermal?.jpegData(compressionQuality: 1)!
